...
namespace RealEstate_Dapper_Api.Repositories.ProductRepository
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            string query = "SELECT TOP (5) ProductId , Title, Price, City,District, ProductCategory,CategoryName, AdvertisementDate FROM Product Inner join Category on Product.ProductCategory = Category.CategoryID WHERE LTRIM(RTRIM([Type])) COLLATE Turkish_CI_AS = N'Kiralık' ORDER BY ProductID DESC;   -- dikkat: ProductID\r\n";
            using (var connection = _context.CreateConnection())
            {
        ...
        public async Task CreateProduct(CreateProductDto createProductDto)
        {
            string query = "insert into Product " +
                "(Title,Price,City,District,CoverImage,Address,Description,Type,DealOfTheDay,AdvertisementDate,ProductStatus,ProductCategory,EmployeeID) values " +
        ...
        public async Task<List<ResultProductWithCategoryDto>> GetProductByDealOfTheDayWithCategoryAsync()
        {
